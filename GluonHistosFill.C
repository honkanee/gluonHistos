...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH2D* gPtResp_hist = new TH2D("gluon_pt_resp_hist", "", NBINS, ptrange, 500, 0.75, 1.25);
   TH2D* gPtRespNGenPF_w_hist = new TH2D("gluon_pt_resp_nGenJetPF_w_hist", "", NBINS, ptrange, 500, 0.75, 1.25);
   TH2D* gPtRespNGenPFSig_w_hist = new TH2D("gluon_pt_resp_nGenJetPFSig_w_hist", "", NBINS, ptrange, 500, 0.75, 1.25);
   ...
   TH2D* qPtResp_hist = new TH2D("quark_pt_resp_hist", "", NBINS, ptrange, 500, 0.75, 1.25);
   TH2D* qPtRespNGenPF_w_hist = new TH2D("quark_pt_resp_nGenJetPF_w_hist", "", NBINS, ptrange, 500, 0.75, 1.25);
   TH2D* qPtRespNGenPFSig_w_hist = new TH2D("quark_pt_resp_nGenJetPFSig_w_hist", "", NBINS, ptrange, 500, 0.75, 1.25);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (fabs(jetEta)<1.3 && jetPtOrder<2 && fpclassify(jetPt) == FP_NORMAL && genJetPt > 0) {
         resp = jetPt/genJetPt;
         ...
         if (isPhysG) {
            gPtResp->Fill(jetPt, resp);
            gPtResp_hist->Fill(jetPt, resp);
            gNGenPF->Fill(jetPt, nGenJetPF);
            gPerPtBin->Fill(genJetPt);
            if (gprob_sig > 0 && qprob_sig > 0) {
               w_sig = qprob_sig/gprob_sig;
               gPtRespNGenPFSig_w->Fill(jetPt, resp, w_sig);
               gPtRespNGenPFSig_w_hist->Fill(jetPt, resp, w_sig);
            }
            if (gprob > 0 && qprob > 0) {
               w = qprob/gprob;
               gPtRespNGenPF_w->Fill(jetPt, resp, w);
               gPtRespNGenPF_w_hist->Fill(jetPt, resp, w);
            }
            else w = 0;
            ...
            if (isPhysUDS) {
               qPtResp->Fill(jetPt, resp);
               qPtResp_hist->Fill(jetPt, resp);
               qNGenPF->Fill(jetPt, nGenJetPF);
               qPerPtBin->Fill(genJetPt);
               if (gprob_sig > 0 && qprob_sig > 0) {
                  w_sig = gprob_sig/qprob_sig;
                  qPtRespNGenPFSig_w->Fill(jetPt, resp, w_sig);
                  qPtRespNGenPFSig_w_hist->Fill(jetPt, resp, w_sig);
               }
               if (gprob > 0 && qprob > 0) {
                  w = gprob/qprob;
                  qPtRespNGenPF_w->Fill(jetPt, resp, w);
                  qPtRespNGenPF_w_hist->Fill(jetPt, resp, w);
               }
               else w = 0;
...
void GluonHistosFill::FillWeightHistos(int nptbins, double* ptrange, int npfbins, double* pfrange, int nMassBins, double* massrange)
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (fabs(jetEta)<1.3 && jetPtOrder<2 && fpclassify(genJetPt) == FP_NORMAL && jetPt > 0) {
         if (isPhysG) {
            gNGenPF->Fill(jetPt, nGenJetPF);
